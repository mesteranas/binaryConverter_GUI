...
class main (qt.QMainWindow):
    def __init__(self):
        ...
        self.text.setAccessibleName(_("text or code"))
        layout.addWidget(self.text)
        self.choice=qt.QComboBox()
        self.choice.addItems([_("to binary"),_("to text")])
    ...
    def on_convert(self):
        ...
        if self.choice.currentIndex()==0:
            try:
                ...
                self.result.setText(binary)
            except ValueError:
                self.result.setText(_("Invalid input"))
        else:
            chunks = text.split()  # Split binary string by spaces
